...
export async function getRecommendatedUsers(req, res) {
    try {
        ...
        const currentUser = req.user; // Get the current user from the request object

        const RecommendatedUsers = await User.find({
            $and: [
                { _id: { $ne: currentUserId } }, // Exclude the current user
                { _id: { $nin: currentUser.friends } }, // Exclude users already in friends list
        ...
        console.error('Error in getRecommendatedUsers:', error);
...
export async function getMyFriends(req, res) {
    try {
        const user = await User.findById(req.user._id).populate('friends', 'fullName profilePicture nativeLanguage learningLanguage ');
